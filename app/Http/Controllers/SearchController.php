    ...
    public function __invoke(Request $request)
    {
        ...
        $products = Product::query()
            ...
            ->when(! $useAlgoliaRelevanceOrder, function ($query) use ($sortBy, $sortDirection) {
                $query->orderByInStockFirst()->orderBy($sortBy, $sortDirection);
            })
            ->with([
        ...
        if ($useAlgoliaRelevanceOrder) {
            ...
            $productWithOutFilters = $productWithOutFilters
                ->sort(function (Product $a, Product $b) use ($idOrder): int {
                    $sa = $a->stock > 0 ? 0 : 1;
                    $sb = $b->stock > 0 ? 0 : 1;
                    if ($sa !== $sb) {
                        return $sa <=> $sb;
        ...
        return $this->productListingIndexResponse('base.pages.products.search', [
            'products' => $products,
