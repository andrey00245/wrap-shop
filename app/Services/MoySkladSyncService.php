    ...
    public static function updateOrderPaymentStatus(Order $order): void
    {
        ...
        try {
            ...
            $updatePayload = [
                'state' => [
                    'meta' => [
                        'href' => 'https://api.moysklad.ru/api/remap/1.2/entity/customerorder/metadata/states/your_paid_state_id_here', // Замените на реальный ID статуса "Оплачен"
                        'type' => 'state',
                        'mediaType' => 'application/json'
                    ]
                ]
            ...
            // Также можно добавить атрибут для отметки об оплате
            $updatePayload['attributes'] = [
                [
                    'meta' => [
                        'href' => 'https://api.moysklad.ru/api/remap/1.2/entity/customerorder/metadata/attributes/your_payment_attribute_id_here', // Замените на реальный ID атрибута
                        'type' => 'attributemetadata',
                        'mediaType' => 'application/json'
                    ],
                    'value' => 'paid'
            ...
            if ($updateResponse->successful()) {
                Log::info("Статус заказа #{$order->id} успешно обновлен в МойСклад");
            } else {
