    ...
    showError(message) {
        this.container.innerHTML = `
            <div style="text-align: center; padding: 40px; color: #dc3545;">
                <p>${message}</p>
                <button onclick="location.reload()" style="padding: 10px 20px; background: #007bff; color: white; border: none; border-radius: 8px; cursor: pointer;">
                    Попробовать снова
                </button>
            </div>
        `;
        ...
        if (!this.faqs.length) {
            this.container.innerHTML = `
                <div style="text-align: center; padding: 40px; color: #6c757d;">
                    <p>FAQ не найден</p>
                    <p>Добавьте вопросы через админку</p>
                </div>
            `;
            return;
        }
                ...
                ${this.faqs.map((faq, index) => `
                    <div class="faq-item" data-faq-id="${faq.id}">
                        <div class="faq-question" data-index="${index}">
                            <h3 class="faq-question-text">${this.escapeHtml(faq.question)}</h3>
