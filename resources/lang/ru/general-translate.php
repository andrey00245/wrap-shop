<?php

return [
    'leaders_of_sales' => '<span class="colord">Лидеры</span> продаж',
    'latest' => '<span class="colord">Новые</span> поступления',
    'similar' => '<span class="colord">Похожие</span> товары',
    '3m_colors' => '<span class="colord">3M color</span> пленки',
    'yellotools' => '<span class="colord">Yellotools</span> инструменты',
    'our_realizations' => 'Плёнки на автомобиле',
    'show_more' => 'Показать больше',

    'sales_hit' => 'Хит продаж',
    'home_block_categories_sr_only' => 'Категории товаров',
    'home_block_kits_title' => '<span class="colord">Готовые</span> решения с плёнкой',
    'home_block_kits_sr_only' => 'Готовые решения с плёнкой',
    'home_block_kits_lead' => 'Наши готовые решения с плёнкой для самостоятельной оклейки позволяют легко и быстро обновить внешний вид вашего авто. Каждый набор включает высококачественную плёнку и подробную инструкцию, которая поможет вам добиться идеального результата дома',
    'all' => 'Все',
    'view' => 'Смотреть',

    'product_card' => [
        'code' => 'Код: ',
        'add_to_cart' => 'Купить',
        'remove' => 'Удалить',
        'add_wishlist' => 'Добавить в закладки',

    ],

];
